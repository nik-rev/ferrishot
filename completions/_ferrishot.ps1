...
Register-ArgumentCompleter -Native -CommandName 'ferrishot' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'ferrishot' {
            ...
            [CompletionResult]::new('--log-level', '--log-level', [CompletionResultType]::ParameterName, 'Choose a miniumum level at which to log')
            [CompletionResult]::new('--log-file', '--log-file', [CompletionResultType]::ParameterName, 'Path to the log file')
            [CompletionResult]::new('--log-filter', '--log-filter', [CompletionResultType]::ParameterName, 'Filter for specific Rust module or crate, instead of showing logs from all crates')
            ...
            [CompletionResult]::new('--log-stderr', '--log-stderr', [CompletionResultType]::ParameterName, 'Log to standard error instead of file')
            [CompletionResult]::new('--debug', '--debug', [CompletionResultType]::ParameterName, 'Launch in debug mode (F12)')
